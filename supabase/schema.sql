...
CREATE INDEX idx_checkins_has_question ON public.checkins(has_question) WHERE has_question = true;
CREATE INDEX idx_checkins_mood ON public.checkins(mood);
...
CREATE INDEX idx_staff_notes_user_id ON public.staff_notes(user_id);
...
-- 質問があるチェックインが作成されたとき、自動でquestion_statusesを作成
...
BEGIN
  ...
  FROM dates
  WHERE grp = (
    SELECT grp FROM dates ORDER BY date DESC LIMIT 1
  );

  -- 質問有無
  SELECT EXISTS(
    SELECT 1 FROM public.checkins WHERE user_id = p_user_id AND has_question = true
  ) INTO v_has_question;

  -- できたこと有無
  SELECT EXISTS(
    SELECT 1 FROM public.checkins WHERE user_id = p_user_id AND done_text != ''
  ) INTO v_has_done;

  -- 成果報告有無
  SELECT EXISTS(
    SELECT 1 FROM public.achievements WHERE user_id = p_user_id
